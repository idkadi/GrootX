...
const slotsPath = path.join(__dirname, "../data/layouts/slots.json");
...
function getBackgroundData(page) {
  ...
  return (
    backgrounds.find(bg => Number(bg.id) === savedBgId) ||
    backgrounds.find(bg => Number(bg.id) === 0)
  );
}

async function generateAlbumPage(album, page, pageNumber) {
  const slotsData = loadJSON(slotsPath);
  const allCards = getCardsArray();

  const layoutId = String(page.layout);
  const slotPositions = slotsData[layoutId];
  ...
  const bgPath = path.join(backgroundsPath, bgData.file);

  if (!fs.existsSync(bgPath)) {
    throw new Error(`Background image not found: ${bgData.file}`);
  }
  ...
  for (let i = 0; i < slotPositions.length; i++) {
    ...
    const cardInfo = allCards.find(
      c => Number(c.id) === Number(placed.cardId)
    );

    if (!cardInfo) continue;

    const cardPath = path.join(imagesPath, cardInfo.image);
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!albumName) {
      return message.reply("❌ Use: `!viewalbum <album name>`");
    }
    ...
    let album = await albumsCol.findOne({
      userId,
      name: {
        $regex: `^${albumName}$`,
        $options: "i"
    ...
    if (!album.pages || album.pages.length === 0) {
      return message.reply("❌ This album has no pages.");
    }
    ...
    if (!page.layout) {
      return message.reply("❌ Page 1 has no layout.");
    }
    ...
    collector.on("collect", async interaction => {
      if (interaction.user.id !== message.author.id) {
        return interaction.reply({
          content: "❌ This album viewer is not for you.",
          ephemeral: true
        });
      }
      ...
      const currentPage = freshAlbum.pages[pageIndex];

      if (!currentPage.layout) {
        return msg.edit({
          content: `❌ Page ${pageIndex + 1} has no layout.`,
          embeds: [],
          files: [],
          components: []
        });
